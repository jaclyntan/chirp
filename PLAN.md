# Chirp — Local Wispr Flow Rebuild
...
| Dictionary | `~/Library/Application Support/Chirp/dictionary.json` — `{ "spoken": "replacement" }` |
...
└── scripts/
    └── make_app.sh            # builds Chirp.app bundle + ad-hoc codesign
```

## CLI test modes (headless verification)

- `Chirp --transcribe <audiofile>` — transcribe a file, print raw + formatted text
- `Chirp --selftest` — run TextFormatter unit checks
...
4. `scripts/make_app.sh` produces `Chirp.app`.
...
2. **Accessibility** — System Settings → Privacy & Security → Accessibility →
   add Chirp.app (required for the global hotkey and for ⌘V injection).
